...
class FacultyReturnWindow:
    ...
    def add_frame(self):
        ...
        self.label = Label(self.frame, text="BOOK RETURNING BY FACULTY", fg='black')
        self.label.config(font=("Poppins", 20, 'underline bold'))
        self.label.place(x=175, y=30)

        # use tree view to show details from the table
        self.tr = Treeview(self.frame, columns=('BOOK_ID', 'FACULTY_ID', 'ISSUE_DATE', 'RETURN_DATE'),
                           selectmode="extended")
    ...
    def bounce(self, e):
        ...
        if col == '#4':
            ...
            if res:
                ...
                if d:
                    messagebox.showinfo("Message", "Faculty Book Returned successfully..!")
                self.win.destroy()
                x = Thirdpage
            else:
                self.win.destroy()

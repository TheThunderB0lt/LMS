...
class BookLendingWindow:
    ...
    def __init__(self):
        ...
        self.win.title("| STUDENT BOOKS LENDING | LIBRARY MANAGEMENT SYSTEM |")
    ...
    def add_frame(self):
        ...
        self.label = Label(self.frame, text="STUDENT BOOK LENDING", fg='black')
        ...
        self.btn = Button(self.frame, text='SUBMIT', width=15, bg='light grey', fg='black',
                          font=("Times", 13, "bold"), command=self.BookLending)
        self.btn.place(x=320, y=250)

        self.win.mainloop()

    def BookLending(self):
        id = (
            self.id.get(),
            self.id.get(),
        )
        ...
        if self.bid.get() == '':
            ...
            self.label2.config(text="Please enter Student Id", font=('Poppins', 11, 'bold'))

            # it shows error message if student id is same..and database is locked
        else:
            status, id_status = Database.database.BookLending(data, id)
            if status and id_status:
                k.showinfo("Welcome", "Book given successfully...!")
                self.win.destroy()
            elif status == 1 and id_status == 0:
                k.showinfo("Already 3 Books Issued to this Student_id Try to returning a book to borrow another")
                self.win.destroy()
            else:
